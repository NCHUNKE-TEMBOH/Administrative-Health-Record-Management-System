...
// Blockchain integration functions for the main application
window.AppBlockchain = {
    // Add health record to blockchain
    addHealthRecord: function(recordData) {
        return fetch('/api/blockchain/add', {
            method: 'POST',
    ...
    getChain: function() {
        return fetch('/api/blockchain/chain')
            .then(response => response.json());
    },

    // Validate blockchain
    validateChain: function() {
        return fetch('/api/blockchain/validate')
            .then(response => response.json());
    },

    // Get patient records from blockchain
    getPatientRecords: function(patientId) {
        return fetch(`/api/blockchain/patient/${patientId}`)
            .then(response => response.json());
    },

    // Get records by type from blockchain
    getRecordsByType: function(recordType) {
        return fetch(`/api/blockchain/type/${recordType}`)
            .then(response => response.json());
    },

    // Show blockchain status indicator
    showBlockchainStatus: function(success, message) {
        const indicator = document.createElement('div');
        indicator.className = `blockchain-status ${success ? 'success' : 'error'}`;
        indicator.innerHTML = `
            <i class="fas fa-cubes"></i>
            <span>${message || (success ? 'Added to blockchain' : 'Blockchain error')}</span>
        `;

        // Add styles if not already present
        if (!document.getElementById('blockchain-status-styles')) {
            const style = document.createElement('style');
            style.id = 'blockchain-status-styles';
            style.textContent = `
                .blockchain-status {
                    position: fixed;
                    top: 20px;
